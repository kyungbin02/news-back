    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if ("naver".equals(registrationId)) {
            ...
            if (user == null) {
                ...
                // 신규가입 알림 전송
                try {
                    notificationService.notifyNewUser(username);
                    System.out.println("[CustomOAuth2UserService] 네이버 신규가입 알림 전송 완료 - username: " + username);
                } catch (Exception e) {
                    System.err.println("[CustomOAuth2UserService] 네이버 신규가입 알림 전송 실패: " + e.getMessage());
                }
            } else {
                // 기존 사용자: UPDATE
            ...
            if (user == null) {
                ...
                try {
                    notificationService.notifyNewUser(username);
                    System.out.println("[CustomOAuth2UserService] 카카오 신규가입 알림 전송 완료 - username: " + username);
                } catch (Exception e) {
                    System.err.println("[CustomOAuth2UserService] 카카오 신규가입 알림 전송 실패: " + e.getMessage());
                }
            ...
            if (user == null) {
                ...
                try {
                    notificationService.notifyNewUser(username);
                    System.out.println("[CustomOAuth2UserService] 구글 신규가입 알림 전송 완료 - username: " + username);
                } catch (Exception e) {
                    System.err.println("[CustomOAuth2UserService] 구글 신규가입 알림 전송 실패: " + e.getMessage());
                }
